...
export class AgentEditComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit() {
    ...
    let mqttHostSplit = this._mqttBroker.host.split(':');
    let mqttHostAddress = mqttHostSplit[1].substr(2);
    let serverHostSplit = GlobalStorageService.SERVER_URL.split(':');
    if (mqttHostAddress.startsWith('localhost')) mqttHostAddress = serverHostSplit[1].substr(2);
    this._agentConfig.SERVER_ADDRESS = '\'' + serverHostSplit[0] + ':' + serverHostSplit[1] + '\'';
    this._agentConfig.SERVER_PORT = '\'' + serverHostSplit[2] + '\'';
    this._agentConfig.AGENT_ID = '\'' + this._agent.id + '\'';
    this._agentConfig.AGENT_TOKEN = '\'' + this._agent.authToken + '\'';
    this._agentConfig.MQTT_BROKER_ADRESS = '\'' + mqttHostAddress + '\'';
    this._agentConfig.MQTT_BROKER_PORT = '\'' + mqttHostSplit[2] + '\'';
